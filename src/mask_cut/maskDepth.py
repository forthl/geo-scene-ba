...
def find_optimal_k(data, max_k):
    ...
    kn = KneeLocator(range(1, max_k+1), distortions,
...
def kmeans_clustering(data, k, current_num_instances):
    ...
    # resolution is hard coded at (320,320)
    instance_mask = np.zeros((320, 320))
    for index, point in enumerate(data):
        instance_mask[int(point[0]), int(point[1])] = kmeans.labels_[
            index]+current_num_instances

    colorMask = grayscale_to_random_color(
        instance_mask, k+current_num_instances).astype(np.uint8)
