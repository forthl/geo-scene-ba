...
from eval_segmentation import batched_crf, debug_apply_crf, debug_batched_crf_first_100, debug_apply_crf_first_two, \
    debug_apply_crf_all_instances
    ...
    with Pool(cfg.num_workers + 5) as pool:
        for i, batch in enumerate(tqdm(loader)):
            with torch.no_grad():
               ...
               # cluster_crf = debug_apply_crf(pool, img, cluster_probs).argmax(1).cuda()
               # cluster_crf = debug_batched_crf_first_100(pool, img, cluster_probs).argmax(1).cuda()
               # cluster_crf = batched_crf(pool, img, cluster_probs).argmax(1).cuda()
               # cluster_crf = debug_apply_crf_first_two(pool, img, cluster_probs).argmax(1).cuda()
                cluster_crf = debug_apply_crf_all_instances(pool, img, cluster_probs).argmax(1).cuda()
                ...
                for depthIdx, point_cloud in enumerate(point_clouds):
                    ...
                    labels, instance_mask = maskD.DBSCAN_clustering(point_cloud, image_shape=image_shape, epsilon=epsilon, min_samples=min_samples)
